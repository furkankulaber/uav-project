...
import type { Event, NewEvent } from '@interfaces/calendar'
...
export const useCalendar = (event: Ref<Event | NewEvent>, isEventHandlerSidebarActive: Ref<boolean>, isLeftSidebarOpen: Ref<boolean>, itemId: Ref<string>) => {
  // 👉 themeConfig
  ...
  watch(() => store.state.calendarStore.events, () => {
    refetchEvents()
  }, { deep: true })

  // 👉 Fetch events  const fetchEvents = computed(() => store.state.calendarStore.events)
  ...
  const fetchEvents: EventSourceFunc = async (info, successCallback) => {
    ...
    successCallback(store.state.calendarStore.events)
  }
  ...
  const calendarOptions = {
    ...
    datesSet({ view }) {
      store.dispatch('calendarStore/setDateFilter', {
